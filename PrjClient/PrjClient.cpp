...
#define DRAWSQUA	1002	//메세지 타입: 사각형
#define DRAWCURC	1003	//메세지 타입: 원
#define DRAWTRAG	1004	//메세지 타입: 직각삼각형
#define DRAWRHOM	1005	//메세지 타입: 마름모
#define DRAWISTR	1007	//메세지 타입: 이등변삼각형
...
/*추가 구현*/
static CURC_MSG		 g_drawcurc; //원 그리기 메시지 저장
static TRAG_MSG		 g_drawtrag; //원 그리기 메시지 저장
static RHOM_MSG		 g_drawrhom; //원 그리기 메시지 저장
static int	g_drawtype;  //클라이언트 그리기 타입
static int  now_drawtype; //받은 그리기 타입



static HWND hCombo;
TCHAR* text[] = { TEXT("선"), TEXT("사각형"), TEXT("원"), TEXT("직각삼각형"), TEXT("이등변삼각형"), TEXT("마름모")};
int index;
...
BOOL CALLBACK DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		hCombo = GetDlgItem(hDlg, IDC_COMBOBOX); //콤보박스 핸들 얻기
		hId = GetDlgItem(hDlg, IDC_ID); //아이드 핸들러 얻기
	...
	case WM_COMMAND:
		...
		case IDC_CONNECT:
			...
			if (GetDlgItemText(hDlg, IDC_ID, g_chatmsg.id, sizeof(g_chatmsg.id) )== NULL) {
				MessageBox(hDlg, "아이디를 입력해 주세요", "실패!", MB_ICONERROR);
			}
			else {
				// 소켓 통신 스레드 시작
		...
		case IDC_COMBOBOX: //도형 선택을 위한 콤보 박스
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_DRAWIT:
		...
		else if (now_drawtype == DRAWTRAG) {//직각 삼각형
		...
		else if (now_drawtype == DRAWRHOM) {//마름모
