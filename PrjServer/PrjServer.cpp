...
int main(int argc, char* argv[])
{
	...
	while (1) {
			...
			|| NetworkEvents.lNetworkEvents & FD_WRITE)
		{
			...
			if (ptr->recvbytes == 0) {
				// 데이터 받기
				retval = recv(ptr->sock, (char*)&len, sizeof(int), 0);
				...
				retval = recv(ptr->sock, ptr->buf, len, 0);
				if (retval == SOCKET_ERROR) {
					if (WSAGetLastError() != WSAEWOULDBLOCK) {
						err_display("recv()");
						RemoveSocketInfo(i);
				...
				for (int j = 1;j < nTotalSockets;j++) {
					ptr2 = SocketInfoArray[j];
